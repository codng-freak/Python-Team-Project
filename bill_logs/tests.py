...
class ImportsTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.details_urls = reverse('import_bill')
    ...
    def test_details_import(self):
        response = self.client.get(self.details_urls)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'import_bill.html')
...
#         e=self.create_Import()
#         self.assertTrue(isinstance(e,imported))
>>>>>>> efdb711387eb5197c676438605cb0e7c7ad49db9
